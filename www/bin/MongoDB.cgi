...
print_mongo_server() {
  ...
  echo "<div class=\"server\" id=\"${nodeid}\">"
    ...
    if [ `echo "$report" | grep -c '<***>'` -eq 0 ] ; then
      ...
      echo "<div class=\"status status_short statuserr clickable\" title="`echo "$report" | grep '<***>'`" id=\"${nodeid}_status\" onclick=\"showDetails('${nodeid}_status','MongoDB/mongostatus')\">Error</div>"
    fi
